 ...
 * @returns {MxObject[]}
 */
function GetStorageItemObjectList(key, entity) {
	// BEGIN USER CODE
  if (!key) {
  ...
  return getItem(key).then(function (result) {
    ...
    return Promise.all(values.map(function (value) {return getOrCreateMxObject(entity, value);})).then(function (newObjects) {
      var newValues = newObjects.map(function (newObject) {return serializeMxObject(newObject);});
      return setItem(key, JSON.stringify(newValues)).then(function () {return newObjects;});
    });
  });
